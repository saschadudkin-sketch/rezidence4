...
const artifactPath = path.join(artifactDir, 'test-e2e-v1-service-execution.json');
const frontendPort = process.env.E2E_FRONTEND_PORT || '3132';
const backendPort = process.env.E2E_BACKEND_PORT || '3133';
...
function runWithCrashRetry(args, env) {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    console.warn(`[test:e2e:v1-service-execution] child process crashed before verdict; retrying (${attempt}/3); status=${status}`);
  }
  return status;
}
