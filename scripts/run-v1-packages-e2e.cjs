...
function runWithCrashRetry(args, env) {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    console.warn(`[test:e2e:v1-packages] child process crashed before verdict; retrying (${attempt}/3); status=${status}`);
  }
  return status;
}
...
function writeArtifact(status) {
  ...
  fs.writeFileSync(artifactPath, `${JSON.stringify({
    schema_version: 1,
    script: 'test:e2e:v1-packages',
    command: 'npm run test:e2e:v1-packages',
