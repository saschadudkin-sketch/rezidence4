  ...
  return (
    <>
      <section className="admin-stats-section">
        <SectionHeader title="Оперативная статистика" />
      ...
      <section className="admin-stats-section">
        <SectionHeader title="SLA и качество обработки" />
...
const AdminRequestsView = memo(function AdminRequestsView({ requests, adminUid }) {
  ...
  const activeFilterChips = [
    reqPeriod !== 'all' ? `Период: ${reqPeriod === 'today' ? 'Сегодня' : 'Неделя'}` : null,
    reqType !== 'all' ? `Тип: ${reqType === 'pass' ? 'Пропуска' : 'Техзаявки'}` : null,
    reqStatus !== 'all' ? `Статус: ${reqStatus}` : null,
  ].filter(Boolean);
  ...
  return (
    <>
      <div className="admin-toolbar">
        ...
        <div className="admin-filter-toggle-wrap">
          <button
            className={'btn-outline admin-filter-toggle' + (filtersOpen ? ' active' : '')}
            onClick={() => setFiltersOpen((v) => !v)}
            aria-expanded={filtersOpen}
            aria-controls="admin-requests-filters"
          >
            Фильтры {filtersOpen ? '▴' : '▾'}
          </button>
        </div>
        {activeFilterChips.length > 0 && (
          <div className="admin-active-filters" aria-label="Активные фильтры">
            {activeFilterChips.map((chip) => (
              <span key={chip} className="admin-active-filter-chip">{chip}</span>
            ))}
          </div>
        )}
      </div>
      {filtersOpen && (
        <div id="admin-requests-filters" className="admin-filters-drawer">
          <div className="date-pills u-mb0">
