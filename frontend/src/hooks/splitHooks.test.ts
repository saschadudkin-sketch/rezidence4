...
vi.mock('../services/pushNotification', () => ({ subscribePush: vi.fn() }));
vi.mock('../utils', () => ({ sendNotif: vi.fn(), playAlert: vi.fn() }));

describe('useDashboardHooks barrel re-exports', () => {
  test('all 6 hooks exported from barrel', async () => {
    const barrel = await import('./useDashboardHooks');
  ...
  test('hooks are importable directly from their own files', async () => {
    expect(typeof (await import('./useTheme')).useTheme).toBe('function');
    expect(typeof (await import('./useNavBadges')).useNavBadges).toBe('function');
    expect(typeof (await import('./useLiveSync')).useLiveSync).toBe('function');
    expect(typeof (await import('./usePushNotifications')).usePushNotifications).toBe('function');
    expect(typeof (await import('./useArrivalNotifier')).useArrivalNotifier).toBe('function');
    expect(typeof (await import('./useNavigation')).useNavigation).toBe('function');
  });

  test('barrel and direct import resolve to the same function', async () => {
    const barrel  = await import('./useDashboardHooks');
    const direct  = await import('./useTheme');
