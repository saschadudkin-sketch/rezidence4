...
describe('requestsReducer', () => {
  ...
  test('REQUEST_SET_STATUS changes status', () => {
    const action: any = { type: 'REQUEST_SET_STATUS', id: 'r1', status: 'approved' };
  ...
  test('REQUEST_ARRIVE sets arrivedAt', () => {
    const now = new Date();
    const action: any = { type: 'REQUEST_ARRIVE', id: 'r1', arrivedAt: now };
    const result = requestsReducer(baseState, action);
  ...
  test('REQUEST_ADD adds new request', () => {
    ...
    const action: any = { type: 'REQUEST_ADD', request: newReq };
    const result = requestsReducer(baseState, action);
  ...
  test('REQUEST_DELETE removes request', () => {
    const action: any = { type: 'REQUEST_DELETE', id: 'r1' };
    const result = requestsReducer(baseState, action);
  ...
  test('HISTORY_ADD appends history entry', () => {
    const action: any = { type: 'HISTORY_ADD', reqId: 'r1', byName: 'Охрана', byRole: 'security', label: 'Допуск разрешён', at: new Date() };
    const result = requestsReducer(baseState, action);
