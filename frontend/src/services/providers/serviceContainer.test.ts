...
describe('serviceContainer smoke', () => {
  afterEach(() => {
    ...
    vi.doUnmock('../../config/runtimeMode');
  });
  ...
  test('builds services via createServices on module init', async () => {
    ...
    vi.doMock('./createServices', () => ({ createServices }));
    const { services: mockedServices } = await import('./serviceContainer');
  ...
  test('keeps singleton export across repeated imports', async () => {
    ...
    vi.doMock('./createServices', () => ({ createServices }));
    const firstImport = await import('./serviceContainer');
    const secondImport = await import('./serviceContainer');
