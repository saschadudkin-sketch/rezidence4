 ...
 */
// FIX [MED-1]: явная типизация параметров — было implicit any.
export async function sendChatMessage({
  remotePayload,
  localMessage,
  sendLocal,
}: {
  remotePayload: Partial<ChatMessage>;
  localMessage: ChatMessage;
  sendLocal: (msg: ChatMessage) => void;
}): Promise<string> {
  if (isLiveMode()) {
    try {
      ...
      logger.warn('[chatGateway] sendMessage failed, falling back to local', (e as Error).message);
