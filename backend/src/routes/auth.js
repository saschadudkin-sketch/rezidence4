    ...
    if (!matchedId) {
      ...
      if (_redis) {
        try {
          ...
          const fails = await _redis.incr(failKey);
          if (fails === 1) await _redis.expire(failKey, OTP_VERIFY_LOCKOUT_SEC);
          if (fails >= OTP_VERIFY_LOCKOUT_MAX) {
            await _redis.set(lockKey, '1', 'EX', OTP_VERIFY_LOCKOUT_SEC);
            await _redis.del(failKey);
