...
// GET /api/v1/residents?unit_id=&q=&is_active=&limit=&offset=
router.get('/', async (req, res, next) => {
  try {
    ...
    const { rows } = await getDb(req).query(
      `SELECT * FROM residents ${where} ORDER BY full_name ASC LIMIT $${limitIdx} OFFSET $${offsetIdx}`,
      params,
    );
    const show = canViewPhone(req);
    res.json({
      residents: rows.map((r) => formatResident(r, show)),
