...
describe('analytics aggregation service', () => {
  test('flattens operations dashboard metrics into export-ready rows', () => {
    ...
    expect(rows).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({
        metric_group: 'access',
        metric_key: 'access.by_access_point.denial_count',
        value: 2,
        dimensions: { access_point_id: 'point-1', name: 'КПП Север' },
      }),
      expect.objectContaining({
        metric_group: 'access',
        metric_key: 'access.offline_replay_by_status',
        value: 2,
        dimensions: { replay_status: 'accepted' },
      }),
      expect.objectContaining({
