...
| Pilot-To-Production Hardening | `DH-41` to `DH-49` | Partial | SKUD provider config, hardware mapping, integration event foundations, first Hikvision + Bolid/Orion-compatible event/sync paths and video evidence reference baseline now exist; deeper vendor rollout, E2E gates and ops tooling remain. |
...
| `DH-43` | Video Evidence Integration | Improved, backend baseline added | `video_evidence_references`, `/api/v1/access-incidents/:id/video-evidence`, `/api/v1/video-evidence/:id`, camera listing over mapped `skud_hardware_devices`, sensitive link/view audit events and no-biometrics validation now exist. Native VMS playback and automatic clip fetching remain out of scope/future work. |
...
| `DH-43` | Improved, backend baseline added | Added link-only video evidence storage, incident/list/view routes, camera mapping reads over SKUD hardware devices, sensitive audit events for link/view and schema/service-level no-biometrics enforcement. | Continue with `DH-44` ERP / 1C / ЖКХ Exchange Baseline recheck. |
...
2. Continue with `DH-44` ERP / 1C / ЖКХ Exchange Baseline recheck.
