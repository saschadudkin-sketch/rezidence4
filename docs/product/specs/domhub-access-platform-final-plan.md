...
Это платформа, которая управляет полным жизненным циклом доступа на объект:
- кто получает доступ;
- на каком основании;
- в какие зоны;
- на какой срок;
...
- ожидаемых гостей;
- активные пропуска;
- поиск по ФИО, QR, авто, квартире, подрядчику;
...
1. Сначала собрать `Access MVP` для одного объекта.  
2. Затем усилить guard, vehicle, contractor and incident layers.  
3. Затем добавить zones, policies и company-level visibility.  
4. После этого строить integrations со СКУД.  
5. И только потом добавлять сложную automation и advanced analytics.
